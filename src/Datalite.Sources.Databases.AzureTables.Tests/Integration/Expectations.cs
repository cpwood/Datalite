using System;
using System.Collections.Generic;
using Datalite.Destination;
using Datalite.Testing;
using FluentAssertions;

namespace Datalite.Sources.Databases.AzureTables.Tests.Integration
{
    internal static class Expectations
    {
        internal static SqliteColumn PartitionKeyColumn = new("PartitionKey", StoragesClasses.StorageClassType.TextClass, true);
        internal static SqliteColumn RowKeyColumn = new("RowKey", StoragesClasses.StorageClassType.TextClass, true);
        internal static SqliteColumn TimestampColumn = new("Timestamp", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn IdColumn => new("id", StoragesClasses.StorageClassType.IntegerClass, false);
        internal static SqliteColumn FirstNameColumn => new("first_name", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn LastNameColumn => new("last_name", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn EmailColumn => new("email", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn GenderColumn => new("gender", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn ImageColumn => new("image", StoragesClasses.StorageClassType.TextClass, false);
        internal static SqliteColumn SalaryColumn => new("salary", StoragesClasses.StorageClassType.RealClass, false);
        
        internal static SqliteColumn[] AllColumns => new[]
        {
            PartitionKeyColumn,
            RowKeyColumn,
            TimestampColumn,
            IdColumn,
            FirstNameColumn,
            LastNameColumn,
            EmailColumn,
            GenderColumn,
            ImageColumn,
            SalaryColumn
        };

        internal static Dictionary<string, object> TableRecord1 = new()
        {
            { "PartitionKey", "Weinham" },
            { "RowKey", "1" },
            { "id", (long)1 },
            { "first_name", "Shoshana" },
            { "last_name", "Weinham" },
            { "email", "sweinham0@cafepress.com" },
            { "gender", "Female" },
            {
                "image",
                ""
            },
            { "salary", 19000.28D }
        };

        internal static Dictionary<string, object> TableRecord2 = new()
        {
            { "PartitionKey", "Hundal" },
            { "RowKey", "2" },
            { "id", (long)2 },
            { "first_name", "Coraline" },
            { "last_name", "Hundal" },
            { "email", "chundal1@webeden.co.uk" },
            { "gender", "Female" },
            {
                "image",
                ""
            },
            { "salary", 38185.39D }
        };

        internal static Dictionary<string, object> TableRecord3 = new()
        {
            { "PartitionKey", "Murton" },
            { "RowKey", "3" },
            { "id", (long)3 },
            { "first_name", "Calv" },
            { "last_name", "Murton" },
            { "email", DBNull.Value },
            { "gender", "Non-binary" },
            {
                "image",
                ""
            },
            { "salary", 22776.57D }
        };

        internal static Dictionary<string, object> TableRecord4 = new()
        {
            { "PartitionKey", "Reedshaw" },
            { "RowKey", "4" },
            { "id", (long)4 },
            { "first_name", "Toinette" },
            { "last_name", "Reedshaw" },
            { "email", DBNull.Value },
            { "gender", "Female" },
            {
                "image",
                ""
            },
            { "salary", 28294.83D }
        };

        internal static string[] PartitionIndex = { "PartitionKey" };
        internal static string[] PartitionRowIndex = { "PartitionKey", "RowKey" };

        internal static AndConstraint<SqliteTableAssertions> MeetTheTableConditions(
            this SqliteTableAssertions tableAssertions)
        {
            var table = tableAssertions.Table;

            return table
                .Should()
                .Exist().And
                .HaveAColumnCountOf(AllColumns.Length).And
                .HaveColumns(AllColumns).And
                .HaveARowCountOf(1000).And
                .HaveAnIndexOf(PartitionIndex).And
                .HaveAnIndexOf(PartitionRowIndex).And
                .HaveARowMatching(TableRecord1).And
                .HaveARowMatching(TableRecord2).And
                .HaveARowMatching(TableRecord3).And
                .HaveARowMatching(TableRecord4);
        }

        internal static AndConstraint<SqliteTableAssertions> MeetTheQueryConditions(
            this SqliteTableAssertions tableAssertions)
        {
            var table = tableAssertions.Table;

            return table
                .Should()
                .Exist().And
                .HaveAColumnCountOf(AllColumns.Length).And
                .HaveColumns(AllColumns).And
                .HaveARowCountOf(3).And
                .HaveAnIndexOf(PartitionIndex).And
                .HaveAnIndexOf(PartitionRowIndex).And
                .HaveARowMatching(TableRecord1).And
                .HaveARowMatching(TableRecord2).And
                .HaveARowMatching(TableRecord3);
        }
    }
}
